...
const validateAddCompany = Joi.object({
  rutCompany: Joi.string().max(12).required(),
  nameCompany: Joi.string().min(6).max(255).required(),
  sizeCompany: Joi.string().valid('Micro', 'Pequeña', 'Mediana', 'Grande').required(),
  idRETC: Joi.optional(),
  statusRETC: Joi.optional(),
  statusValidationCompany: Joi.boolean().optional()
});

export { validateRegister, validateLogin, validateResendValidationCode, validateAddUser, validateAddCompany }